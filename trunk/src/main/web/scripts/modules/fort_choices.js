...
$(function() {
    m3.fort_choices = function() {
        ...
        fort_choices.custom_forts = [];
        
        // This sets the custom fort to be loaded into the level. Soon it will also validate the
        // fort data. The fort should be passed in as stringified JSON.
        fort_choices.setCustomFort = function(fort, player) {
            var parsed_fort;
            
            // If we receive invalid JSON, just return.
            try {
                parsed_fort = JSON.parse(fort);
            }
            catch(error) {
                return;
            }
            
            this.custom_forts[player] = parsed_fort;
        };
